...
struct OCTOPUS_EXPORT state_interpolation_data {
	...
	state_interpolation_data(octree_client const& s, face d, array<boost::int64_t, 3> o) :
			subject(s), direction(d), offset(o) {
	}

	state_interpolation_data(octree_client const& s, face d) :
	...
	bool operator<(state_interpolation_data const& rhs) const {
		...
		return std::make_pair(strip_credit_from_gid(lhs_gid), direction) < std::make_pair(strip_credit_from_gid(rhs_gid), rhs.direction);
	}
...
struct OCTOPUS_EXPORT flux_interpolation_data {
	...
	bool operator<(flux_interpolation_data const& rhs) const {
		return std::make_pair(idx, direction) < std::make_pair(rhs.idx, rhs.direction);
	}
...
struct OCTOPUS_EXPORT octree_server: hpx::components::managed_component_base<octree_server> {
private:
	friend struct oid_type;

	typedef hpx::components::managed_component_base<octree_server> base_type;

	typedef hpx::components::managed_component<octree_server>* back_pointer_type;
...
//    atomic_bitset<8> marked_for_refinement_;
	...
	typedef array<hpx::lcos::local::channel<void>, 6> sibling_sync_dependencies;
	...
	boost::uint8_t get_flux_index(octopus::axis a, boost::uint8_t l, boost::uint8_t cj, boost::uint8_t ck) {
	...
	void parent_to_child_injection(vector4d<double> const& pU);
...
//        hpx::id_type gid = get_gid();
		OCTOPUS_ASSERT(hpx::invalid_id != this_);OCTOPUS_ASSERT_MSG(
...
public:
	...
	octree_server(back_pointer_type back_ptr, octree_init_data const& init);

	/// \brief Construct a child node.
	octree_server(back_pointer_type back_ptr, octree_init_data const& init, boost::shared_ptr<vector4d<double> > const& parent_U);
	...
	state& operator()(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) {
		return (*U_)(i, j, k);
	}

	state const& operator()(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) const {
		return (*U_)(i, j, k);
	}
	...
	array<double, 3> center_coords(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) const {
	...
	array<double, 3> x_face_coords(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) const {
		array<double, 3> coords;
		coords[0] = x_face(i);
		coords[1] = y_center(j);
		coords[2] = z_center(k);
	...
	array<double, 3> y_face_coords(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) const {
		array<double, 3> coords;
		coords[0] = x_center(i);
		coords[1] = y_face(j);
		coords[2] = z_center(k);
	...
	array<double, 3> z_face_coords(boost::uint64_t i, boost::uint64_t j, boost::uint64_t k) const {
		array<double, 3> coords;
		coords[0] = x_center(i);
		coords[1] = y_center(j);
		coords[2] = z_face(k);
	...
	void set_time(double time, boost::uint64_t step);
...
//      , boost::uint64_t disparity ///< Difference in refinement level
	...
	///////////////////////////////////////////////////////////////////////////
	// Child -> parent injection of multipole moments
	...
	HPX_DEFINE_COMPONENT_ACTION(octree_server,
			...
			child_to_parent_multipole_action);
...
private:
	...
	void child_to_parent_multipole_kernel(
			boost::uint64_t phase
	);
...
inline oid_type::oid_type(octree_server const& e) :
		level_(e.level_), location_(e.location_), gid_(e.reference_from_this()) {
}
