    ...
    ) 
{
    ...
    for (boost::uint64_t i = 0; i < 8; ++i)
        ...
        {
            keep_alive.emplace_back
                (children_[i].template reduce_zonal_ordered_async<T>
                    (f, reducer, initial)); 
...
template <typename T>
inline T octree_client::reduce_zonal_ordered(
    hpx::util::function<T(state&)> const& f
  , hpx::util::function<T(T const&, T const&)> const& reducer
  , T const& initial 
    ) const 
{
    return reduce_zonal_ordered_async<T>(f, reducer, initial).get();
}

template <typename T>
inline hpx::future<T> octree_client::reduce_zonal_ordered_async(
    hpx::util::function<T(state&)> const& f
  , hpx::util::function<T(T const&, T const&)> const& reducer
  , T const& initial
    ) const
{
    ensure_real();
    typedef octopus::octree_server::reduce_zonal_ordered_action<T> action_type;
    return hpx::async<action_type>(gid_, f, reducer, initial); 
}
